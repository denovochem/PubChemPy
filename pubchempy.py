...
#: Dummy CID for instances where a cid could not be resolved by the ID Exchange Service
#: This CID should be very uncommon/unlikely to be quiered - we use a rare carbon isotope by default
DUMMY_CID = '58965162'
    ...
    for i in range(0, len(identifiers), chunk_size):
        ...
        if req_id_elem is None:
            ...
            if error_elem is not None:
                error_msg = root.find(".//PCT-Status-Message_message")
...
def create_batch_properties_request_xml(
    ...
    if identifier_type.lower() == 'name':
        ...
        for identifier in identifiers:
            ET.SubElement(inchikey_elem, "PCT-QueryUids_inchi-keys_E").text = str(identifier)
    else:
...
def poll_request_status(
    ...
    while time.time() - start_time < timeout:
        ...
        if status == 'success':
            ...
            error_message = error_elem.text if error_elem is not None else "Unknown error"
            return None
...
def get_compounds(
    ...
    **kwargs: QueryParam,
    ...
    compounds = [compound if str(compound.cid) != DUMMY_CID else None for compound in compounds]
    if as_dataframe:
